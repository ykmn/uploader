Uploader 2.07.012
==================
12.07.2018 Roman Ermakov <r.ermakov@emg.fm>
...
* RDS-кодер DEVA SmartGen с передачей данных по TCP или UDP.
...
* Производится замена текста в строках **Artist** и **Name** по "хэш-таблице замены" - удаляются строки *PI_*, *ID_EP LIVE*, *NEW_*, *MD_*, *EDIT_*; нижние подчёркивания заменяются на пробел; два пробела заменяются на один; заменяются исключения в регистре букв типа *ABBA*, *LP*, *DJ*, *OneRepublic*, *McCartney*
...
* Если значение типа текущего элемента равно *1* (реклама), то строка радиотекста составляется из значений адреса сайта и коммерческой информации из файла конфигурации. Значение RT+ при этом обнуляется.
...
* Значение типа текущего элемента сравнивается с "последним удачным" значением типа. Если оно одинаковое (несколько джинглов или рекламных роликов подряд), то загрузка радиотекста в RDS прекращается.
  * Если в скрипте установлен флаг $force = $true, то программа форсированно продолжает работу.
* Иначе в RDS-кодер с ip-адресом и портом из файла конфигурации отправляются строка RT и RT+; при значении RDSPORTTYPE=UDP в файле конфигурации строка отправляется в указанный порт по не по TCP, а по UDP; значение типа текущего элемента сохраняется как "последнее удачное".
...
- v2.07.010 2017-12-26 количество найденных следующих песен ограничено двумя; добавлено использование русских A/T из пользовательских атрибутов.
- v2.07.011 2018-03-06 добавлена поддержка передачи RDS в кодер Deva SmartGen по UDP.
>>>>>>> 87d2eb661d4191df8cad1a660f6b070fc9dc7a3d

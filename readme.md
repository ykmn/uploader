...
=======
* Если в значении **Artist** встречается *;* то значение **Artist** делается пустым (""), подразумевая что правильная форма нескольких исполнителей *First Artist feat. Second and Third Artist* должна быть записана в поле **Name**.
* Если в МБД используются пользовательские атрибуты (User Attributes) "Русский исполнитель" и "Русское название", то необходимо заменить или добавить соответствующие им ID 7 и 17 в строках скрипта:
>>>>>>> c36b0a0bcaba36b9f6bbd107fb571beb104fbf4b
...
* Производится замена текста в строках **Artist** и **Name** по "таблице замены" - удаляются строки *PI_*, *ID_EP LIVE*, *NEW_*, *MD_*, *EDIT_*, нижние подчёркивания заменяются на пробел, два пробела заменяются на один, заменяются исключения в регистре букв типа *ABBA*, *LP*, *DJ*, *McCartney*

* Если текущий элемент (**ELEM_0**) песня и его статус *Playing* и значение **dbID** не равно *null*, то готовый массив сортируется по полю ELEM, конвертируется в JSON и сохраняется в json-файл с текущим временем скрипта **$scriptstart**.
>>>>>>> c36b0a0bcaba36b9f6bbd107fb571beb104fbf4b
...
<<<<<<< HEAD
* Если текущий элемент (ELEM_0) имеет статус *Playing* и его тип "песня", то собирается строка для Omnia ProStream в формате "Artist - Name", а если тип "не песня", то строка будет пустой ("").
...
Если оно одинаковое (несколько джинглов или рекламных роликов подряд), то отправка метаданных в ProStream прекращается, иначе в ProStream с ip-адресом и портом из файла конфигурации отправляются текстовая строка в формате фильтра "Character Parser Sample" и значение типа текущего элемента сохраняется как "последнее удачное".
>>>>>>> c36b0a0bcaba36b9f6bbd107fb571beb104fbf4b

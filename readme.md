Uploader 2.07.013a
==================
01.08.2018 Roman Ermakov <r.ermakov@emg.fm>
...
* Если значение типа текущего элемента равно *1* (реклама), то строка радиотекста составляется из значений "коммерческая информаци" `RDSCOMMERCIAL` и "адрес сайте" `RDSSITE` из файла конфигурации. Значение RT+ при этом обнуляется.

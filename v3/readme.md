Uploader 3.03.001
==================
11.07.2022 Roman Ermakov <r.ermakov@emg.fm>
...
* Один или два FTP-сервера, на которые выгрузится XML-файл с метаданными от DJin.
...
Для использования RDS-кодера, укажите его модель (`RDSDEVICE=SmartGen` или `RDSDEVICE=8700i` или `RDSDEVICE=LinkShare`) и тип отправки данных (`RDSPORTTYPE=UDP` или `RDSPORTTYPE=TCP`). UDP работает только для DEVA SmartGen.
...
* Если в файле конфигурации задано действие `FTP1=TRUE`, и текущий элемент (ELEM_0) имеет статус *Playing*, и среди установленных модулей PowerShel есть WinSCP, то выполняется отправка локальной копии XML-файла под оригинальным именем на FTP.
* Если в файле конфигурации задано действие `FTP2=TRUE`, то выполняется отправка локальной копии XML-файла на второй FTP.

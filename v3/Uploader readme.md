...
> 22.04.2025 Roman Ermakov <r.ermakov@emg.fm>
...
* RDS: отправка транслитерированной строки Artist-Title в RDS-кодер DEVA SmartGen, Orban 8700 или в LinkShare.
...
### Пример файла конфигурации example.cfg.json:
...
Если категорию нужно временно отключить, переименуйте её:
```json
    "ftp-disabled": [
        {
            "pass": "password",
...
* Если в файле конфигурации задано действие `POST`, и текущий элемент (ELEM_0) имеет статус *Playing*, то выполняется отправка содержимого XML-файла через POST на веб-адрес, указанный в файле конфигурации.

* Если в файле конфигурации задано действие `JSONLOCAL`, и текущий элемент (ELEM_0) имеет статус *Playing*, то в папку ./jsons/ сохраняется файл **myradio.cf.local.json** с полями "artist - title", "artist", "title" и "dbID".

* Если в файле конфигурации задано действие `JSONCUSTOM`, и текущий элемент (ELEM_0) имеет статус *Playing* и тип *Музыка*, то генерируется кастом-файл в формате json, содержащий информацию о текущей и двух следующих песням, и он методом POST отправляется на веб-адрес, указанный в файле конфигурации.
...
V3.06.000 2024-12-23 BREAKING CHANGE: файл конфигурации теперь в JSON; обработка неограниченного количества получателей в каждой категории; изменен метод отправки по FTP.

V3.06.002 2024-12-23 BREAKING CHANGE: изменены параметры файла конфигурации; исправлены опечатки.

V3.06.004 2025-04-22 исправлена кириллица в AT для jsonlocal; кириллица в ProStream теперь отправляется транслитом.

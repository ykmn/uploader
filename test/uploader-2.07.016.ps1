...
.LINK
    https://github.com/ykmn/uploader/v2/blob/master/readme.md
...
.PARAMETER test
    If $test set to $true then we didn't compare jsons and forcing push to webserver and RDS
...
#####################################################################################
Write-Host "Uploader 2.07.016 <r.ermakov@emg.fm> 2020-01-20 https://github.com/ykmn/uploader"
...
$now = Get-Date -Format HH:mm:ss.fff
Add-Content -Path $log -Value "$now : $scriptstart Now Playing: $type/ $artist - $title"
...
#            Invoke-Command -ScriptBlock {Invoke-WebRequest -Uri $jsonserver -Method POST -Body $json -ContentType "application/json"} -AsJob
            ...
            Write-Host "JSON push engaged. AT: $artist $title Element:"$xmlfile.root.ELEM_0.Elem.FONO_INFO.Type.'#text'", Status:"$xmlfile.root.ELEM_0.Status
            $now = Get-Date -Format HH:mm:ss.fff
            Add-Content $log "$now : $scriptstart [+] JSON $forced push engaged. AT: $artist $title Element: $($xmlfile.root.ELEM_0.Elem.FONO_INFO.Type.'#text'), Status: $($xmlfile.root.ELEM_0.Status), JSON=$($h.Get_Item('JSON'))"
        } catch { 
        ...
        $now = Get-Date -Format HH:mm:ss.fff
        Add-Content -Path $log -Value "$now : $scriptstart JSON $forced push didn't engaged. Element: $($xmlfile.root.ELEM_0.Elem.FONO_INFO.Type.'#text'), Status: $($xmlfile.root.ELEM_0.Status), JSON=$($h.Get_Item('JSON'))"
    }

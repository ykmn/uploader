...
v1.05 2020-02-26 cyrillic files are renaming from $ConfigTable.Xml to $ConfigTable.Dst
...
Write-Host "`nFileMonitor.ps1   v1.04 2020-02-26"
...
function Write-Log {
    ...
    $logfile = $currentdir + "\log\" + $(Get-Date -Format yyyy-MM-dd) + "-" + $(Split-Path $MyInvocation.PSCommandPath -Leaf) + ".log"
        ...
        if ($change.TimedOut -eq $false)
        {
            ...
            for ($n=0; $n -lt $ConfigTable.count; $n++)
            {
                if ($ConfigTable[$n].Xml -like $pn)
                {
                    ...
                    Write-Log -message "$p corresponds to $cfgFM and uploader $execute" -color Green

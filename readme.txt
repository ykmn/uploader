...
1. Для применения процедуры выгрузки на FTP необходимо установить модуль WinSCP для PowerShell:
	# сохранить
...
3. Создайте файл конфигурации по примеру ниже и сохраните его в "myradio.cfg" в папку со скриптом.
...
В файле конфигурации строки, начинающиеся с [ и # не обрабатываются, их можно использовать как комментарии.
...
4. Файл runps.bat нужен для запуска PowerShell-скрипта из Джина.ValueServer. Этот файл должен находиться в рабочей папке с PS-скриптом,
...
* PS-скрипт запускается из Джин.ValueServer через "костыль" runps.bat с именем файла конфигурации в видео обязательного параметра. Например: runps myradio.cfg
...
* Значение Runtime переводится из миллисекунд в секунды и округляется в бОльшую сторону.
